...
const op1Precedes = (op1, op2) => {
  ...
  const op1Value = _formulaTokenizer.operatorOrder.findIndex(finder(op1));

  const op2Value = _formulaTokenizer.operatorOrder.findIndex(finder(op2));
...
const convertToRPN = tokens => {
  ...
  tokens.forEach(token => {
    ...
    if (requiresPlaceholder(token)) {
      if (operatorStack.length > 0 && (0, _helpers.peek)(operatorStack) !== '(') {
        log(`result << ops.pop('${(0, _helpers.peek)(operatorStack)}')`);
    ...
    if ((0, _formulaTokenizer.isOperator)(token)) {
      while (operatorStack.length > 0 && op1Precedes(token, (0, _helpers.peek)(operatorStack))) {
        log(`result << ops.pop('${(0, _helpers.peek)(operatorStack)}')`);
        result.push(operatorStack.pop());
      }
      ...
      while (operatorStack.length > 0 && (0, _helpers.peek)(operatorStack) !== '(') {
        log(`result << ops.pop('${(0, _helpers.peek)(operatorStack)}')`);
        result.push(operatorStack.pop());
      }

      log(`ops.pop ('${(0, _helpers.peek)(operatorStack)}')`);
      operatorStack.pop();
    } else {
      log(`result << '${token}'`);
      result.push(token);
...
var _default = convertToRPN;
exports.default = _default;
//# sourceMappingURL=rpnConverter.js.map