 ...
 */
const validateParentheses = (clause) => {
  let depth = 0;
 ...
 * ensure it alternates between operators and values
 *
 */
const validateTokens = tokens => tokens.every(isValidToken);
 ...
 */
const clauseHasMislocatedAssignmentOperator = clause => clause.some(
  (token, i) => token === '='
    && (clause.length < 3
      || i !== 1
...
export {
  clauseHasMislocatedAssignmentOperator,
  validateClause,
  validateParentheses,
}
