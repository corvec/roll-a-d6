 ...
 * @typedef Collection
 * @type {object}
 * @property {object.<string,string>} rolls Addressable, named rolls/values
 * @property {object.<string,Collection>} collections Child collections, cannot yet be addressed
 */
 ...
 * @function convertToFormulasMap
 * @param {object.<string,Collection>} targetedCollections
 * @returns {object.<string,string>}
 */
const convertToFormulasMap = (targetedCollections) => {
  const formulasMap = Object.entries(targetedCollections)
    .reduce((accum, [collectionName, collection]) => ({
      ...accum,
      ...(
        Object.entries(collection.rolls)
          .map(([rollName, { formula }]) => [`${rollName}@${collectionName}`, formula])
 ...
 * @param {string} formula - The base formula to roll
 * @param {object.<string,string>} collectionFormulasMap - Any helper functions that are available
 * @param {object.<string,Collection>} targetedCollections - Map of collection names to collection
 *        objects. For targeting.
 * @returns {RolledFormula}
 */
export const rollFormula = (formula, collectionFormulasMap, targetedCollections = {}) => {
