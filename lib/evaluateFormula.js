...
const log = (...args) => {
  if (logEvaluate) console.log(...args);
};
/**
...
const getInitialRollIndex = sides => sides.map(side => [side, 0]).reduce(_helpers.objectMakerReduceHelper, {});

const rollIsSaved = (sides, evaluationMetadata) => typeof evaluationMetadata.rollIndex[sides] === 'number' && evaluationMetadata.rollIndex[sides] < evaluationMetadata.rolls[sides].length;
...
const isMacro = (token, macros) => macros.hasOwnProperty((0, _formulaTokenizer.stripPrefix)(token));
/**
 * @function
...
const getMacroName = token => {
  const target = (0, _formulaTokenizer.variableTargetsCollection)(token) ? (0, _formulaTokenizer.getTargetCollection)(token) : null;
  return `${(0, _formulaTokenizer.stripPrefix)((0, _formulaTokenizer.stripSuffix)(token))}${target ? `@${target}` : ''}`;
};
/**
 * @function
 * @example // returns '1'
...
const isMacroInstance = (token, macros) => {
  if (/[a-zA-Z]\w*\[\d+]/.test((0, _formulaTokenizer.stripPrefix)(token))) {
...
const applyEvaluation = (expansionOperator, values) => {
  const {
    operator,
    initialValue
  } = getExpansionOperatorData(expansionOperator);
...
const evaluateExpansionOperator = ({
  ...
  if ((0, _formulaTokenizer.isRoll)(repeatedValue) || isMacro(repeatedValue, evaluationMetadata.macros)) {
    const expandedValues = [stack.slice(-1)[0], ...[...Array(repetitionCount - 1)].map(() => (0, _formulaTokenizer.isRoll)(repeatedValue) ? rollRoll(repeatedValue, evaluationMetadata) : evaluateExpression(evaluationMetadata.macros[getMacroName(repeatedValue)], evaluationMetadata))];
...
const evaluateValue = ({
  ...
  const value = (0, _formulaTokenizer.isRoll)(token) ? rollRoll(token, evaluationMetadata) : isMacro(token, macros) ? evaluationMetadata.sideEffects.hasOwnProperty(getMacroName(token)) ? evaluationMetadata.sideEffects[getMacroName(token)] : evaluateExpression(macros[getMacroName(token)], evaluationMetadata) : isGlobalMacroInstance(token, macros) ? getOrEvalMacroInstance(token, evaluationMetadata, evaluationMetadata.savedGlobalValues) : isMacroInstance(token, macros) ? getOrEvalMacroInstance(token, evaluationMetadata, savedMacroValues) : isBoolean(token) ? toBoolean(token) : parseInt(token, 10);
  log(`${token} => ${value}`);
  return [...stack, value];
};
...
const storeCurrentValue = ({
  ...
  evaluationMetadata.sideEffects[variableName] = isMacro(variableName, evaluationMetadata.macros) ? evaluateExpression(evaluationMetadata.macros[variableName], evaluationMetadata) : 0;
};
...
const applySideEffect = ({
  ...
  const val = (0, _helpers.peek)(stack);
...
const evaluateExpression = (expression, evaluationMetadata) => {
  ...
  const result = expression.reduce((stack, token, tokenIndex) => {
    if ((0, _formulaTokenizer.isConditional)(token)) {
      return evaluateConditional({
        stack,
        token,
        evaluationMetadata
      });
    } else if ((0, _formulaTokenizer.isConditional)((0, _helpers.peek)(stack)) || token === '...') {
      return ignoreToken({
        stack
      });
    } else if ((0, _formulaTokenizer.isSideEffectVariable)(token)) {
      return [...stack, token];
    } else if ((0, _formulaTokenizer.isSideEffectOperator)(token)) {
      return applySideEffect({
        stack,
        token,
        evaluationMetadata
      });
    } else if ((0, _formulaTokenizer.isExpansionOperator)(token)) {
      return evaluateExpansionOperator({
        stack,
        token,
        tokenIndex,
        expression,
    ...
    } else if ((0, _formulaTokenizer.isOperator)(token)) {
...
const evaluateFormula = ({
  ...
  const result = expressions.map(expression => evaluateExpression(expression, evaluationMetadata));
  return {
    result,
    rolls: evaluationMetadata.rolls,
    sideEffects: evaluationMetadata.sideEffects
  };
};

var _default = evaluateFormula;
